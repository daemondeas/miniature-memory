...
public class Input22 : AbstractInput<(Dictionary<Tuple<int, int>, Day22.Tile>, Dictionary<int, Tuple<int, int>>, Dictionary<int, Tuple<int, int>>, FSharpList<FSharpChoice<Day22.Turn, int>>), (Dictionary<Tuple<int, int>, Day22.Tile>, Dictionary<int, Tuple<int, int>>, Dictionary<int, Tuple<int, int>>, FSharpList<FSharpChoice<Day22.Turn, int>>)>
{
    protected override (Dictionary<Tuple<int, int>, Day22.Tile>, Dictionary<int, Tuple<int, int>>, Dictionary<int, Tuple<int, int>>, FSharpList<FSharpChoice<Day22.Turn, int>>) ParseInput(string input)
    {
        var rows = input.Split('\n');
        var map = new Dictionary<Tuple<int, int>, Day22.Tile>();
        var xLimits = new Dictionary<int, Tuple<int, int>>();
        var yMin = new Dictionary<int, int>();
        var yMax = new Dictionary<int, int>();
        ...
        for (var i = 0; i < rows.Length - 1; i++)
        {
            ...
            for (var j = 0; j < rows[i].Length; j++)
            {
                ...
                if (rows[i][j] == '.')
                {
                    map[new Tuple<int, int>(j + 1, i + 1)] = Day22.Tile.Open;
                }
                
                if (rows[i][j] == '#')
                {
                    map[new Tuple<int, int>(j + 1, i + 1)] = Day22.Tile.Wall;
                }
            }
            
            xLimits[i + 1] = new Tuple<int, int>(firstOnRow, rows[i].Length);
        }

        var yLimits = new Dictionary<int, Tuple<int, int>>();
        foreach (var x in yMin.Keys)
        {
            yLimits[x] = new Tuple<int, int>(yMin[x], yMax[x]);
        }
        
        return (map, xLimits, yLimits, ParseInstructions(rows.Last()));
    }

    private static FSharpList<FSharpChoice<Day22.Turn, int>> ParseInstructions(string row)
    {
        var lastParsed = -1;
        var result = new List<FSharpChoice<Day22.Turn, int>>();
        for (var i = 0; i < row.Length; i++)
        {
            if (row[i] is 'L' or 'R')
            {
                result.Add(FSharpChoice<Day22.Turn, int>.NewChoice2Of2(int.Parse(row[(lastParsed + 1)..i])));
                result.Add(FSharpChoice<Day22.Turn, int>.NewChoice1Of2(row[i] == 'L' ? Day22.Turn.LeftTurn : Day22.Turn.RightTurn));
                lastParsed = i;
            }
        }

        if (lastParsed != row.Length - 1)
        {
            result.Add(FSharpChoice<Day22.Turn, int>.NewChoice2Of2(int.Parse(row[(lastParsed + 1)..])));
        }

        return ListModule.OfSeq(result);
    }

    protected override (Dictionary<Tuple<int, int>, Day22.Tile>, Dictionary<int, Tuple<int, int>>,
        Dictionary<int, Tuple<int, int>>, FSharpList<FSharpChoice<Day22.Turn, int>>) ParseInputTwo(string input) =>
        ParseInput(input);

    protected override long SolveFirstPuzzle(
        (Dictionary<Tuple<int, int>, Day22.Tile>, Dictionary<int, Tuple<int, int>>,
            Dictionary<int, Tuple<int, int>>, FSharpList<FSharpChoice<Day22.Turn, int>>) input) =>
        Day22.firstPuzzle(input.Item1, input.Item2, input.Item3, input.Item4);

    protected override long SolveSecondPuzzle(
        (Dictionary<Tuple<int, int>, Day22.Tile>, Dictionary<int, Tuple<int, int>>, Dictionary<int, Tuple<int, int>>,
            FSharpList<FSharpChoice<Day22.Turn, int>>) input) =>
        Day22.secondPuzzle(input.Item1, input.Item2, input.Item4);
